...
${titleLines
  .map(
    (line, i) =>
      `<text x="80" y="${titleY + i * lineH}" fill="#b8ffc8" font-family="VT323, monospace" font-size="76" filter="url(#glow)">${escapeXml(line)}</text>
       <text x="80" y="${titleY + i * lineH}" fill="#4eff7c" font-family="VT323, monospace" font-size="76">${escapeXml(line)}</text>`
  )
  .join("\n")}
...
<rect x="1080" y="540" width="40" height="50" fill="#4eff7c" opacity="0.85"/>
</svg>`;

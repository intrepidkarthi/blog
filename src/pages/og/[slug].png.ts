...
export const GET: APIRoute = async ({ props }) => {
  ...
  // Size the title to its length so the full headline fits without truncation.
...
<text x="80" y="100" fill="#5f7a65" font-family="monospace" font-size="18" letter-spacing="3">
  KARTHIKEYAN NG:~/writing$ cat ${escapeXml(slug)}.md
</text>

<line x1="80" y1="135" x2="1120" y2="135" stroke="#16291c" stroke-width="2"/>

<text x="80" y="180" fill="#ffb454" font-family="monospace" font-size="22" letter-spacing="2">
...
${titleLines
  .map(
    (line, i) =>
      `<text x="80" y="${titleY + i * lineH}" fill="#b8ffc8" font-family="monospace" font-size="${fontSize}" filter="url(#glow)">${escapeXml(line)}</text>
       <text x="80" y="${titleY + i * lineH}" fill="#4eff7c" font-family="monospace" font-size="${fontSize}">${escapeXml(line)}</text>`
  )
  .join("\n")}

<line x1="80" y1="540" x2="1120" y2="540" stroke="#16291c" stroke-width="2"/>
<text x="80" y="575" fill="#5fd7ff" font-family="monospace" font-size="20" letter-spacing="2">
  intrepidkarthi.com
</text>
<text x="1120" y="575" fill="#5f7a65" font-family="monospace" font-size="14" letter-spacing="2" text-anchor="end">

...
## Stretch
**Plug your Session 4 `search_notes` in as a tool** — run the *Reload your Lab 4 store* cell first (it mounts Drive and reads back `chunk_vecs.npy` + `chunks.json`, no re-embedding), then your capstone becomes a real assistant (knowledge + hands) · manual agent loop · break-it-then-fix-the-docstring (proves docstrings are prompts).
